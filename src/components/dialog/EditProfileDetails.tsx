...
interface IEditProfileDetails {
  open: boolean;
  handleOpen: () => void;
  handler: (firstName: string, lastName: string, bio: string) => void;
  formData: { firstName: string; lastName: string; bio: string };
  handleChange: (e: React.ChangeEvent<HTMLInputElement>) => void;
}

const EditProfileDetails: React.FC<IEditProfileDetails> = ({
  open,
  handleOpen,
  handler,
  formData,
    ...
    <Dialog open={open} onClose={handleOpen} className="relative z-10">
      ...
      <div className="fixed inset-0 z-10 w-screen overflow-y-auto">
        <form
          ...
          onSubmit={(e) => {
            e.preventDefault();
            handler(formData.firstName, formData.lastName, formData.bio);
          }}
        >
          <DialogPanel
            transition
